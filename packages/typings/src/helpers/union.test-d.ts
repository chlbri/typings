...
// Union of primitives
const unionPrimitives = type(({ union }) => ({ value: union('string', 'number') }));
expectTypeOf(unionPrimitives.type).toEqualTypeOf<{ value: string | number }>();

// Union of three primitives
const unionThree = type(({ union }) => ({
  value: union('string', 'number', 'boolean'),
}));
expectTypeOf(unionThree.type).toEqualTypeOf<{ value: string | number | boolean }>();

// Union with null
const unionNullable = type(({ union }) => ({ nullable: union('string', 'null') }));
expectTypeOf(unionNullable.type).toEqualTypeOf<{ nullable: string | null }>();
...
expectTypeOf(complexDiscriminated.type).toEqualTypeOf<{
  response: { status: string; data: string } | { status: string; error: string };
}>();
