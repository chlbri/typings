...
describe('Helper: litterals', () => {
  describe('#01 => string literals', () => {
    ...
    test('#01 => value matches', () =>
      expect(result.type).toEqual({ status: 'active' }));
  ...
  describe('#02 => number literals', () => {
    const result = type(({ litterals }) => ({ priority: litterals(1, 2, 3) }));

    test('#01 => value matches', () => expect(result.type).toEqual({ priority: 1 }));
  });

  describe('#03 => boolean literals', () => {
    const result = type(({ litterals }) => ({ flag: litterals(true, false) }));
  ...
  describe('#04 => mixed literals', () => {
    ...
    test('#01 => value matches', () =>
      expect(result.type).toEqual({ value: 'yes' }));
  });
