...
expectTypeOf(tupleNumbers.type).toEqualTypeOf<{ coordinates: [number, number] }>();

// Tuple with mixed types
const tupleMixed = type(({ tuple }) => ({
  pair: tuple('string', 'number', 'boolean'),
}));
expectTypeOf(tupleMixed.type).toEqualTypeOf<{ pair: [string, number, boolean] }>();
...
const tupleRgb = type(({ tuple }) => ({ rgb: tuple('number', 'number', 'number') }));
expectTypeOf(tupleRgb.type).toEqualTypeOf<{ rgb: [number, number, number] }>();

// Tuple with string and number
const tupleEntry = type(({ tuple }) => ({ entry: tuple('string', 'number') }));
expectTypeOf(tupleEntry.type).toEqualTypeOf<{ entry: [string, number] }>();
