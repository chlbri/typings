...
describe('Helper: record', () => {
  describe('#01 => record with string value (no keys)', () => {
    const result = type(({ record }) => ({ dict: record('string') }));

    test('#01 => value matches', () => expect(result.type).toEqual({ dict: {} }));

    test(`#02 => ${STANDARD_KEY}.version is 1`, () =>
      expect(result[STANDARD_KEY].version).toBe(1));

    test('#03 => validate() captures the value', () =>
      expect(result[STANDARD_KEY].validate('any')).toEqual({ value: { dict: {} } }));
  });
  ...
  describe('#02 => record with specific keys', () => {
    ...
    test('#01 => value matches', () =>
      expect(result.type).toEqual({
        config: { enabled: 'boolean', visible: 'boolean', active: 'boolean' },
      }));
  });
  ...
  describe('#04 => record with number value (no keys)', () => {
    const result = type(({ record }) => ({ scores: record('number') }));

    test('#01 => value matches', () => expect(result.type).toEqual({ scores: {} }));
  });

  describe('#05 => record with single key', () => {
    const result = type(({ record }) => ({ single: record('string', 'onlyKey') }));

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ single: { onlyKey: 'string' } }));
  });
});
