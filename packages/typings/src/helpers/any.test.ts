...
describe('Helper: any', () => {
  describe('#01 => with string type', () => {
    const result = type(({ any }) => ({ value: any('string') }));
  ...
  describe('#02 => with nested object', () => {
    ...
    test('#01 => value matches', () =>
      expect(result.type).toEqual({ data: { name: 'string', age: 'number' } }));
  ...
  describe('#03 => with number type', () => {
    const result = type(({ any }) => ({ count: any('number') }));
  ...
  describe('#04 => without argument', () => {
    const result = type(({ any }) => ({ unknown: any() }));
  ...
  describe('#05 => with boolean type', () => {
    const result = type(({ any }) => ({ flag: any('boolean') }));
