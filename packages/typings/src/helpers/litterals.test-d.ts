...
// Number literals
const litteralsNumber = type(({ litterals }) => ({ priority: litterals(1, 2, 3) }));
expectTypeOf(litteralsNumber.type).toEqualTypeOf<{ priority: 1 | 2 | 3 }>();

// Boolean literals
const litteralsBoolean = type(({ litterals }) => ({ flag: litterals(true, false) }));
expectTypeOf(litteralsBoolean.type).toEqualTypeOf<{ flag: true | false }>();
...
expectTypeOf(litteralsTwo.type).toEqualTypeOf<{ direction: 'left' | 'right' }>();
