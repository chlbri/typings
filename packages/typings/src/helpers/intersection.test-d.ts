...
const intersectionThree = type(({ intersection }) => ({
  entity: intersection({ id: 'string' }, { name: 'string' }, { createdAt: 'date' }),
}));
...
expectTypeOf(intersectionComplex.type).toEqualTypeOf<{
  item: { id: string; tags: string[]; createdAt: Date; active: boolean };
}>();
...
const social = type({ platform: 'string', url: 'string' });
    ...
    intersection(
      ...
      union.discriminated(
        'personality',
        {
          ...
          nationalID: 'string',
          name: any({ firstName: optional('string'), lastName: optional('string') }),
        },
        {
...
type TIntermediary = {
  ...
  sacrifice?: number;
  contacts: {
    phoneNumbers: { countryCode: number; number: string; network?: string }[];
    emails?: string[];
  ...
  | { personality: 'company'; companyName: string; registrationNumber: string }
);
