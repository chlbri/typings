...
const recordString = type(({ record }) => ({ dict: record('string') }));
expectTypeOf(recordString.type).toEqualTypeOf<{ dict: Record<Keys, string> }>();
...
const recordNumber = type(({ record }) => ({ scores: record('number') }));
expectTypeOf(recordNumber.type).toEqualTypeOf<{ scores: Record<Keys, number> }>();

// Record with single key
const recordSingle = type(({ record }) => ({ single: record('string', 'onlyKey') }));
