...
// Array of strings
const arrayString = type(({ array }) => ({ tags: array('string') }));
expectTypeOf(arrayString.type).toEqualTypeOf<{ tags: string[] }>();

// Array of numbers
const arrayNumber = type(({ array }) => ({ scores: array('number') }));
expectTypeOf(arrayNumber.type).toEqualTypeOf<{ scores: number[] }>();

// Array of booleans
const arrayBoolean = type(({ array }) => ({ flags: array('boolean') }));
...
// Nested arrays
const nestedArray = type(({ array }) => ({ matrix: array(array('number')) }));
