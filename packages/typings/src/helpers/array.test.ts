...
describe('Helper: array', () => {
  describe('#01 => array of strings', () => {
    const result = type(({ array }) => ({ tags: array('string') }));

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ tags: ['string'] }));
  ...
  describe('#02 => array of objects', () => {
    ...
    test('#01 => value matches', () =>
      expect(result.type).toEqual({ users: [{ name: 'string', age: 'number' }] }));
  ...
  describe('#03 => nested arrays', () => {
    const result = type(({ array }) => ({ matrix: array(array('number')) }));

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ matrix: [['number']] }));
  });

  describe('#04 => array of numbers', () => {
    const result = type(({ array }) => ({ scores: array('number') }));

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ scores: ['number'] }));
  });

  describe('#05 => array of booleans', () => {
    const result = type(({ array }) => ({ flags: array('boolean') }));

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ flags: ['boolean'] }));
  });

  describe('#06 => array with nested object', () => {
    const result = type(({ array }) => ({
      items: array({ id: 'string', data: { value: 'number', label: 'string' } }),
    }));
