...
import type { Primitive } from '@bemedev/typings';

describe('helpers: primitive', () => {
  const _default = type(({ primitive }) => primitive());
  expectTypeOf(_default.type).toEqualTypeOf<Primitive>();

  const _default2 = type(({ primitive }) => ({ value: primitive() }));
  expectTypeOf(_default2.type).toEqualTypeOf<{ value: Primitive }>();

  const _default3 = type(({ primitive, readonly }) =>
    readonly({ value: primitive() }),
  );
  expectTypeOf(_default3.type).toEqualTypeOf<{ readonly value: Primitive }>();
  ...
  expectTypeOf(symbolLiteral.type).toEqualTypeOf<{ key: typeof Symbol.iterator }>();

  // primitive.never() → { value: never }
  const neverResult = type(({ primitive }) => ({ value: primitive.never }));
  expectTypeOf(neverResult.type).toEqualTypeOf<{ value: never }>();

  // primitive.undefined() → { value: undefined }
  const undefinedResult = type(({ primitive }) => ({ value: primitive.undefined }));
  expectTypeOf(undefinedResult.type).toEqualTypeOf<{ value: undefined }>();
});
