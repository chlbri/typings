...
import { primitiveObject } from '@bemedev/typings/helpers';

describe('helpers: primitiveObject', () => {
  ...
  expectTypeOf<FlatMapT['type']>().toEqualTypeOf<{ name: string; age: number }>();
  ...
  expectTypeOf<SchemaVarT['type']>().toEqualTypeOf<{ id: string; score: number }>();
  ...
  expectTypeOf(typeWithFlatMap.type).toEqualTypeOf<{ name: string; age: number }>();
  ...
  expectTypeOf<TU1>().toEqualTypeOf<boolean | { name: string } | { age: number }>();
  ...
  const context = pretype(type(({ primitiveObject }) => primitiveObject.const))(
    ({ array, optional, partial, omit, intersection }) => ({
      ...
      creating: optional(
        intersection(omit(allView.__type, 'parent'), { parent: optional('string') }),
      ),

      user: { name: 'string', prefs: {} },

      utilities: partial({
        rawUtility: 'string',
        currents: array({ utility: 'string', active: 'boolean' }),
      }),
  ...
  expectTypeOf(context.type).toEqualTypeOf<{
    ...
    string: string;
    canvasZoom: number;
    user: { name: string; prefs: EmptyObject };
    utilities: {
      rawUtility?: string | undefined;
      currents?: { utility: string; active: boolean }[] | undefined;
    };
    history: {
      views: {
        id: string;
  ...
  const stringContext1 = _stringContext.type(({ litterals }) => litterals('string'));
  ...
  const pCustom = type(({ primitiveObject, custom }) =>
    primitiveObject({ id: custom<string>(), age: custom<number>() }),
  );
