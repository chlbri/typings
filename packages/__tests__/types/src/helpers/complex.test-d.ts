...
import type { inferSh, Keys, SoA, StateValue } from '@bemedev/typings';

describe('helpers: complex', () => {
        ...
        intersection(
          {
            ...
            data: { label: use(label), content: 'string' },
            input: 'boolean',
          },
          { id: 'string' },
        ),
  ...
  const apiResponse = type(({ array, optional, union, intersection }) => ({
    data: union(
      ...
      { success: 'boolean', error: { code: 'number', message: 'string' } },
    ),
    pagination: optional({ page: 'number', total: 'number', hasMore: 'boolean' }),
  }));
  ...
  const pretype1 = _pretype({
    ...
    children: { child1: { NEXT: 'undefined', PREVIOUS: 'undefined' } },
  });
