...
import type { StateValue } from '@bemedev/typings';

describe('helpers: sv', () => {
  // SV basic usage
  const svBasic = type(({ sv }) => ({ state: sv.const }));
  expectTypeOf(svBasic.type).toEqualTypeOf<{ state: StateValue }>();

  // SV in nested object
  const svNested = type(({ sv }) => ({ machine: { currentState: sv.type } }));
  expectTypeOf(svNested.type).toEqualTypeOf<{
    machine: { currentState: StateValue };
  }>();

  // Multiple SV
  const svMultiple = type(({ sv }) => ({ state1: sv.type, state2: sv.type }));
  ...
  const svWithOthers4 = type(({ sv, optional }) => ({
    state: sv({ parallel: { state1: 'state11', state2: 'state22' } }),
    name: 'string',
    count: optional('number'),
  }));
  expectTypeOf(svWithOthers4.type).toEqualTypeOf<{
    state: {
      readonly parallel: { readonly state1: 'state11'; readonly state2: 'state22' };
    };
    name: string;
    count?: number;
  }>();
