import type { StandardSchemaV1 } from '@bemedev/typings';
...
describe('standardize — string', () => {
  ...
  expectTypeOf(s.type).toEqualTypeOf<string>();
  expectTypeOf(s[STANDARD_KEY].version).toEqualTypeOf<1>();
  expectTypeOf(s[STANDARD_KEY].vendor).toEqualTypeOf<string>();
  expectTypeOf(s[STANDARD_KEY].types?.input).toEqualTypeOf<string | undefined>();
  expectTypeOf(s[STANDARD_KEY].types?.output).toEqualTypeOf<string | undefined>();
...
describe('standardize — number', () => {
  ...
  expectTypeOf(s.type).toEqualTypeOf<number>();
  expectTypeOf(s[STANDARD_KEY].types?.input).toEqualTypeOf<number | undefined>();
});

describe('standardize — undefined', () => {
  const s = standardize(undefined);

  expectTypeOf(s.type).toEqualTypeOf<undefined>();
});

describe('standardize — null', () => {
  const s = standardize(null);

  expectTypeOf(s.type).toEqualTypeOf<null>();
});

describe('standardize — object', () => {
  const s = standardize({ a: 1 });

  expectTypeOf(s.type).toEqualTypeOf<{ a: number }>();
...
describe('standardize2', () => {
  ...
  expectTypeOf(standardize2<{ a: number }>({ a: 1 })).toEqualTypeOf<{ a: number }>();
});
