...
describe('type — no option', () => {
  ...
  test('#01 => value is undefined', () => expect(result.type).toBeUndefined());
  ...
  describe('#04 => validate', () => {
    test('#01 => validate() returns { value: undefined }', () =>
      expect(result[STANDARD_KEY].validate('any')).toEqual({ value: undefined }));
...
describe('type — direct primitive string option', () => {
  ...
  describe('#02 => validate', () => {
    test('#01 => validate() returns { value: "string" }', () =>
      expect(result[STANDARD_KEY].validate('any')).toEqual({ value: 'string' }));

    test('#02 => validate(arg) still returns { value: "string" }', () =>
      expect(result[STANDARD_KEY].validate(42)).toEqual({ value: 'string' }));
  });
});
...
describe('type — empty object option', () => {
  ...
  test('#01 => value is {}', () => expect(result.type).toEqual({}));

  test('#02 => validate() returns { value: {} }', () =>
    expect(result[STANDARD_KEY].validate('any')).toEqual({ value: {} }));
});
...
describe('type — function option', () => {
  describe('#01 => returning a plain object via helpers', () => {
    ...
    test('#01 => value.v is "string"', () =>
      expect(result.type).toEqual({ v: 'string' }));
  ...
  describe('#03 => returning a nested object', () => {
    const result = type(() => ({ user: { name: 'string', age: 'number' } }));

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ user: { name: 'string', age: 'number' } }));
...
describe(`type — ${STANDARD_KEY}.types`, () => {
  describe('#01 => with string value', () => {
    const result = type('number');

    test('#01 => types.input is "number"', () =>
      expect(result[STANDARD_KEY].types?.input).toBe('number'));

    test('#02 => types.output is "number"', () =>
      expect(result[STANDARD_KEY].types?.output).toBe('number'));
  });
  ...
  describe('#02 => with object value', () => {
    ...
    test('#01 => types.input matches', () =>
      expect(result[STANDARD_KEY].types?.input).toEqual({ x: 'boolean' }));

    test('#02 => types.output matches', () =>
      expect(result[STANDARD_KEY].types?.output).toEqual({ x: 'boolean' }));
  });
});
...
describe('type — validate always returns captured value', () => {
  ...
  test('#02 => validate(null)', () =>
    expect(result[STANDARD_KEY].validate(null)).toEqual({
      value: { key: 'string' },
    }));

  test('#03 => validate({ key: "other" })', () =>
    expect(result[STANDARD_KEY].validate({ key: 'other' })).toEqual({
      value: { key: 'string' },
    }));

  test('#04 => validate(undefined)', () =>
    expect(result[STANDARD_KEY].validate(undefined)).toEqual({
      value: { key: 'string' },
    }));
});
