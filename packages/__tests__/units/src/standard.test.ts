...
import { standardize, standardize2 } from '@bemedev/typings/standard';

describe('standardize', () => {
  describe('#01 => with a string value', () => {
    const result = standardize('hello');

    test('#01 => value is "hello"', () => expect(result.type).toBe('hello'));
    ...
    describe('#06 => validate', () => {
      test('#01 => validate() returns { value: "hello" }', () =>
        expect(result[STANDARD_KEY].validate('any')).toEqual({ value: 'hello' }));
      ...
      test('#03 => validate(42) still returns { value: "hello" }', () =>
        expect(result[STANDARD_KEY].validate(42)).toEqual({ value: 'hello' }));
    });
  });
  ...
  describe('#04 => with null', () => {
    ...
    test('#03 => validate() returns { value: null }', () =>
      expect(result[STANDARD_KEY].validate('any')).toEqual({ value: null }));
  });
  ...
  describe('#05 => with a boolean', () => {
    ...
    test('#02 => validate() returns { value: true }', () =>
      expect(result[STANDARD_KEY].validate('any')).toEqual({ value: true }));
  });
  ...
  describe('#07 => with an array', () => {
    ...
    test('#01 => value matches', () => expect(result.type).toEqual([1, 2, 3]));

    test('#02 => validate() returns { value: [1,2,3] }', () =>
      expect(result[STANDARD_KEY].validate('any')).toEqual({ value: [1, 2, 3] }));
  });
});
...
describe('standardize2', () => {
  ...
  test('#05 => with null returns null', () => expect(standardize2(null)).toBeNull());
