...
import type { PrimitiveObjectT } from '@bemedev/typings';
...
describe('Helper: primitiveObject', () => {
  ...
  describe('#05 => direct call — nested map', () => {
    const result = primitiveObject({ user: { name: 'string', active: 'boolean' } });

    test('#01 => returns the nested map', () =>
      expect(result).toEqual({ user: { name: 'string', active: 'boolean' } }));
  });
  ...
  describe('#09 => via type() — number literal', () => {
    ...
    test('#01 => value is number string', () => expect(result.type).toBe('number'));
  });
  ...
  describe('#10 => via type() — nested map', () => {
    ...
    test('#01 => value matches', () =>
      expect(result.type).toEqual({ user: { name: 'string', active: 'boolean' } }));
  });
  ...
  test('#12 => Complex', () => {
    ...
    const context = pretype<PrimitiveObjectT>()(
      ({ array, optional, partial, omit, intersection, use }) => ({
        canvas: array('string'),
        views: array(use(allView)),
        dragging: optional(use(allView)),
        string: 'string',
        selectedID: optional(use(allView).id),
        strings: optional(array('string')),
        currentVersion: optional('string'),
        canvasZoom: 'number',

        creating: optional(
          intersection(omit(use(allView), 'parent'), { parent: optional('string') }),
        ),
    ...
    const output = {
      ...
      utilities: {
        rawUtility: 'string',
        currents: [{ utility: 'string', active: 'boolean' }],
      },
      history: [
        {
          views: [
