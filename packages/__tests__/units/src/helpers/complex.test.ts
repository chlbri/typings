...
import { UNION } from '@bemedev/typings/constants';
...
describe('Transform: Complex scenarios', () => {
  ...
  describe('#02 => complex form schema', () => {
    ...
    test('#01 => value.submitUrl is "string"', () =>
      expect(result.type.submitUrl).toBe('string'));

    test('#02 => value.method is array of methods', () =>
      expect(result.type.method).toEqual('GET'));

    test('#03 => value.fields is an array', () =>
      expect(Array.isArray(result.type.fields)).toBe(true));

    test(`#04 => ${STANDARD_KEY}.version is 1`, () =>
      expect(result[STANDARD_KEY].version).toBe(1));
  });

  describe('#03 => API response schema with union', () => {
    const result = type(({ union }) => ({ status: union('string', 'number') }));

    test('#01 => value.status has UNION marker', () =>
      expect(result.type).toEqual({ status: { [UNION]: ['string', 'number'] } }));
  });
  ...
  describe('#04 => deeply nested readonly', () => {
    ...
    test('#02 => value.nickname is "string"', () =>
      expect(result.type.nickname).toBe('string'));

    test('#03 => value.tags is "string"', () =>
      expect(result.type.tags).toBe('string'));

    test(`#04 => ${STANDARD_KEY}.version is 1`, () =>
      expect(result[STANDARD_KEY].version).toBe(1));

    test("#05 => value.status is 'active'", () =>
      expect(result.type.status).toBe('active'));
  });
});
