...
describe('Helper: sv (StateValue)', () => {
  describe('#01 => sv.const usage', () => {
    const result = type(({ sv }) => ({ state: sv.const }));
  ...
  describe('#02 => sv.type usage in nested object', () => {
    const result = type(({ sv }) => ({ machine: { currentState: sv.type } }));
  ...
  describe('#03 => multiple sv', () => {
    const result = type(({ sv }) => ({ state1: sv.type, state2: sv.type }));
