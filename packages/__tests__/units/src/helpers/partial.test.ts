...
describe('Helper: partial', () => {
  ...
  describe('#02 => partial with single property', () => {
    ...
    test('#01 => value matches', () =>
      expect(result.type).toEqual({ config: { enabled: 'boolean' } }));
  });
  ...
  describe('#03 => partial with multiple properties', () => {
    ...
    test('#01 => value matches', () =>
      expect(result.type).toEqual({
        settings: { theme: 'string', fontSize: 'number', darkMode: 'boolean' },
      }));
  });

  describe('#04 => partial at root', () => {
    const result = type(({ partial }) => partial({ id: 'string', name: 'string' }));
