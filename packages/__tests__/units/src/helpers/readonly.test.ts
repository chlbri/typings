...
describe('readonly', () => {
  describe('#01 => wrapping a flat object', () => {
    ...
    test('#01 => value matches', () => expect(result.type).toEqual({ x: 'string' }));
  ...
  describe('#02 => inside an object', () => {
    ...
    test('#01 => value matches', () =>
      expect(result.type).toEqual({ r: { name: 'string', age: 'number' } }));
  ...
  describe('#03 => wrapping a nested object', () => {
    const result = type(({ readonly }) => readonly({ outer: { inner: 'number' } }));
