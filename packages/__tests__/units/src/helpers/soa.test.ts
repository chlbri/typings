...
describe('Helper: soa (SingleOrArray)', () => {
  describe('#01 => soa with string', () => {
    const result = type(({ soa }) => ({ value: soa('string') }));
  ...
  describe('#02 => soa with object', () => {
    const result = type(({ soa }) => ({ item: soa({ name: 'string' }) }));
  ...
  describe('#03 => soa with number', () => {
    const result = type(({ soa }) => ({ count: soa('number') }));
  ...
  describe('#04 => soa with boolean', () => {
    const result = type(({ soa }) => ({ flag: soa('boolean') }));
  ...
  describe('#06 => soa without argument', () => {
    const result = type(({ soa }) => ({ data: soa() }));
