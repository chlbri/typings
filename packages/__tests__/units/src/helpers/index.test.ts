import { type } from '@bemedev/typings';
import { PRIMITIVES } from '@bemedev/typings/constants';
import { STANDARD_KEY } from '@bemedev/typings/constants';

describe('Transform: type() function', () => {
  describe('#01 => no argument', () => {
    const result = type();

    test('#01 => value is undefined', () => expect(result.type).toBeUndefined());

    test(`#02 => ${STANDARD_KEY}.version is 1`, () =>
      expect(result[STANDARD_KEY].version).toBe(1));

    test('#03 => validate() returns { value: undefined }', () =>
      expect(result[STANDARD_KEY].validate('any')).toEqual({ value: undefined }));
  });

  describe('#02 => primitive string literals', () => {
    it.each(PRIMITIVES.map(k => [k] as const))(
      '#0%# => type("%s") returns schema with value',
      input => {
        const result = type(input as any);
        expect(result.type).toBe(input);
        expect(result[STANDARD_KEY].version).toBe(1);
      },
    );
  });

  describe('#03 => plain object', () => {
    const result = type({});

    test('#01 => value is empty object', () => expect(result.type).toEqual({}));

    test(`#02 => ${STANDARD_KEY}.version is 1`, () =>
      expect(result[STANDARD_KEY].version).toBe(1));
  });

  describe('#04 => object with properties', () => {
    const result = type({ name: 'string', age: 'number' });

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ name: 'string', age: 'number' }));

    test('#02 => validate() captures the value', () =>
      expect(result[STANDARD_KEY].validate('any')).toEqual({
        value: { name: 'string', age: 'number' },
      }));
  });

  describe('#05 => recursive object', () => {
    const result = type({ user: { name: 'string', active: 'boolean' } });

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ user: { name: 'string', active: 'boolean' } }));
  });
});
