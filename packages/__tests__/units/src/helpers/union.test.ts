...
describe('Helper: union', () => {
  describe('#01 => union of two primitives', () => {
    const result = type(({ union }) => ({ value: union('string', 'number') }));

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ value: { [UNION]: ['string', 'number'] } }));
  ...
  describe('#05 => union with null', () => {
    const result = type(({ union }) => ({ nullable: union('string', 'null') }));

    test('#01 => value matches', () =>
      expect(result.type).toEqual({ nullable: { [UNION]: ['string', 'null'] } }));
  });
});
