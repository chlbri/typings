...
describe('Helper: sora (SingleOrArray)', () => {
  describe('#01 => sora with string', () => {
    const result = type(({ sora }) => ({ value: sora('string') }));
  ...
  describe('#02 => sora with object', () => {
    const result = type(({ sora }) => ({ item: sora({ name: 'string' }) }));
  ...
  describe('#03 => sora with number', () => {
    const result = type(({ sora }) => ({ count: sora('number') }));
  ...
  describe('#04 => sora with boolean', () => {
    const result = type(({ sora }) => ({ flag: sora('boolean') }));
  ...
  describe('#06 => sora without argument', () => {
    const result = type(({ sora }) => ({ data: sora() }));
