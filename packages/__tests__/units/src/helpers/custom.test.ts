...
describe('Helper: custom', () => {
  describe('#01 => custom with no argument', () => {
    const result = type(({ custom }) => ({ value: custom<number>() }));
  ...
  describe('#02 => custom with string value', () => {
    const result = type(({ custom }) => ({ value: custom('test') }));
  ...
  describe('#04 => custom with RegExp type', () => {
    const result = type(({ custom }) => ({ regex: custom<RegExp>() }));
  ...
  describe('#05 => custom with array type', () => {
    const result = type(({ custom }) => ({ items: custom<string[]>() }));
