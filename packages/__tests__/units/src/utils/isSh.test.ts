...
import { isSh } from '@bemedev/typings/utils/isSh';
...
describe('isSh', () => {
  ...
  describe('#02 => with non-object values', () => {
    test('#01 => string returns false', () => expect(isSh('hello')).toBe(false));
    ...
    test('#06 => array returns false', () => expect(isSh([1, 2, 3])).toBe(false));
  });
  ...
  describe('#05 => version property issues', () => {
    ...
    test('#02 => version is not a number returns false', () => {
      const obj = {
        ...base,
        [STANDARD_KEY]: { ...base[STANDARD_KEY], version: '1' },
      };
      expect(isSh(obj)).toBe(false);
    });
  });
  ...
  describe('#06 => vendor property issues', () => {
    ...
    test('#02 => vendor is not a string returns false', () => {
      const obj = {
        ...base,
        [STANDARD_KEY]: { ...base[STANDARD_KEY], vendor: 123 },
      };
      expect(isSh(obj)).toBe(false);
    });
  });
  ...
  describe('#07 => types property issues', () => {
    ...
    test('#02 => types is not an object returns false', () => {
      const obj = {
        ...base,
        [STANDARD_KEY]: { ...base[STANDARD_KEY], types: 'string' },
      };
      expect(isSh(obj)).toBe(false);
    });
    ...
    test('#03 => types is null returns false', () => {
      const obj = {
        ...base,
        [STANDARD_KEY]: { ...base[STANDARD_KEY], types: null },
      };
      expect(isSh(obj)).toBe(false);
    });
    ...
    test('#04 => types missing input returns false', () => {
      const obj = {
        ...base,
        [STANDARD_KEY]: { ...base[STANDARD_KEY], types: { output: 'string' } },
      };
      expect(isSh(obj)).toBe(false);
    });
    ...
    test('#05 => types missing output returns false', () => {
      const obj = {
        ...base,
        [STANDARD_KEY]: { ...base[STANDARD_KEY], types: { input: 'string' } },
      };
      expect(isSh(obj)).toBe(false);
    });
  });
  ...
  describe('#08 => validate property issues', () => {
    ...
    test('#02 => validate is not a function returns false', () => {
      const obj = {
        ...base,
        [STANDARD_KEY]: { ...base[STANDARD_KEY], validate: 'not a function' },
      };
      expect(isSh(obj)).toBe(false);
    });
  });
  ...
  describe('#10 => with complex input/output types', () => {
    const validSh = {
      ...
      [STANDARD_KEY]: {
        version: 1,
        vendor: '@bemedev/typings',
        types: { input: { complex: 'object' }, output: ['array', 'of', 'values'] },
        validate: () => ({ value: 'test' }),
      },
    };
